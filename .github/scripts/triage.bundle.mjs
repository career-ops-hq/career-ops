...
// {"builtAt":"2026-09-22T20:15:57.448Z","core":"bin/lib/triage-core.mjs","policies":{"labels":"8 entradas","trivial":"16 entradas","priority":"7 entradas"}}
...
//      JEV_API_KEY (+ JEV_URL) opcionales → snapshot.signals; si faltan o fallan, null (`jev:unavailable`).
...
async function collectSignals(snapshot) {
  const key = process.env.JEV_API_KEY, url = process.env.JEV_URL;
  if (!key || !url) return null; // sin Jev: suelo determinista
  try {
    const res = await fetch(url, { method: 'POST', headers: { authorization: `Bearer ${key}`, 'content-type': 'application/json' },
      body: JSON.stringify({ pr: snapshot.number, title: snapshot.title, body: snapshot.body.slice(0, 4000), files: snapshot.files.map((f) => f.path).slice(0, 100) }) });
    if (!res.ok) throw new Error(String(res.status));
    return await res.json();
  } catch (e) { log(`jev #${snapshot.number}: unavailable (${e.message.slice(0, 60)})`); return null; }
}
export async function buildSnapshot(number) {
  ...
  snap.signals = await collectSignals(snap);
  return snap;
}
...
async function triageOne(number, { now, firstReply }) {
...
async function main() {
  ...
  if (eventName === 'schedule' || eventName === 'workflow_dispatch') await sweep(now);
    ...
    await triageOne(event.issue.number, { now, firstReply: false });
