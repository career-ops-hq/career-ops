...
**URL cell rendering (#3516):** in the **TSV the `url` field is always a raw URL** — that boundary is deliberate, because the first-party web writes those TSVs and never reads the tracker's URL cell. `merge-tracker.mjs` renders the cell it writes into `applications.md` as a markdown link with a short label — the ATS vendor on a multi-tenant board (`[ashby](https://jobs.ashbyhq.com/…)`), otherwise the host (`[careers.snowflake.com](https://…)`) — because a raw posting URL runs to 141 characters and pushes every other column off screen. **The href is the dedup key, so every reader must extract it before normalizing:** `normalizeUrl('[label](url)')` returns `''`, and `''` means "this row has no URL", so a reader that misses the link form does not error — it silently drops the row to fuzzy company+role matching. Use `extractCellUrl` (`tracker-parse.mjs`) — `parseTrackerRow` already applies it, so `row.url` is always the href. Rows are converted as merges rewrite them; convert a whole existing tracker with `node merge-tracker.mjs --migrate-urls [--dry-run]` (idempotent, never touches an href, and leaves a hand-written label alone).
