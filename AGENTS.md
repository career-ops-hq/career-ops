...
| Wants a hiring-manager's read on a tailored CV before sending | `pdf` — its optional HM audit pass (`modes/pdf/hm-audit.md`) researches the likely reviewer, dispatches a separate agent role-playing them, and returns a bullet-by-bullet keep/cut/rewrite verdict |
