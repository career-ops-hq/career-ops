...
| `prescore.mjs` | Zero-token heuristic pre-score for one JD (title / must-have requirements / occupation / comp vs `config/profile.yml` + `cv.md`), so `pipeline` spends no LLM call on an obvious no; opt-in via `pipeline.prescore.enabled`, never touches the 1-5 score. A signal with no evidence scores 4, a skip needs evidence, comp alone never rejects, the `modes/_brief.md` priority list always proceeds; `--log` appends one three-field row to `data/discard.log`; a non-zero exit means PROCEED (JSON or `--summary`) |
