...
Two root-level files predate the convention and are the exception: `test-trust-validator.mjs` and `test-salary-filter.mjs` are named `test-*.mjs`, which the discovery glob does not match, so they stay registered by hand in `test-all.mjs`'s `SCRIPTS` list. Discovery imports a suite in-process and shares its counters; a `node:test` suite is child-processed instead, so its results survive ([#2828](https://github.com/santifer/career-ops/issues/2828)). A discovered suite therefore reports through `pass`/`fail` from `tests/helpers.mjs` and never calls `process.exit()` or `finish()`, both of which would forge the run's verdict.
