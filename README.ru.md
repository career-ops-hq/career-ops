...
**Telegram-каналы** подключаются как обычный источник сканера через `provider: telegram-channel` (читает публичное превью `t.me/s/<канал>` — без ключей и логина, с историей через `max_pages`), образец записи — в `templates/portals.example.yml`. Например: @devjobs, @remote_it. Приватные каналы и каналы с выключенным превью так не читаются — сканер сообщит об этом явно.
