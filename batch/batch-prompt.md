...
11. **Immigration-Status Requirement Overreach** — derive the candidate's jurisdiction key from `config/profile.yml` → `location` and read `templates/immigration-status-requirements.yml`. No entry → say nothing. When the JD demands a status beyond what the entry allows, quote the posting's wording and pair it with that entry's `lawful_screening_contrast` and `legal_basis`; never conclude the employer is in breach.
