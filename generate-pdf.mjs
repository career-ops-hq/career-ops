...
async function generatePDF() {
  ...
  // Rewrite <img src="..."> paths pointing into fonts/ to absolute file:// URLs.
  // Playwright's page.setContent() does NOT honor the baseURL option, so the document
  // loads as about:blank and relative <img src> paths can't resolve. Mirror the font
  // rewrite above for images so CV avatars render in the PDF.
  // Matches: src="./fonts/X", src="../fonts/X", src="../../fonts/X", src="fonts/X"
  html = html.replace(
    /src=(['"])(?:\.\.?\/)*(fonts\/[^'"]+)\1/g,
    `src=$1file://${__dirname}/$2$1`
  );
