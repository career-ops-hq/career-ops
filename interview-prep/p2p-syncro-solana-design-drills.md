...
| 2 | Reward Distribution Pipeline | Rung 4 (guided full pass) | ~3.5/5 - strong-senior pass | Failure-mode reasoning (again); reached for in-memory monitor instead of durable state | Crux = **exactly-once payout over at-least-once submission, surviving a crash**. Arrived at the correct answer but only after being pinned 4x: **write-ahead persistence** (persist signature BEFORE submit - closes the crash-between-submit-and-record window) + **on-chain reconciliation** by signature on restart. Sharpened: persist batch MEMBERSHIP `{(delegator,amount)[], signature, blockhash, status}` not just signature (else unrebuildable / can't tell who's owed); reconcile at delegator level. Compute phase must be deterministic/checkpointed or persisted signatures don't map back. Rebuild-and-resign only when tx definitively dead (failed=landed+reverted=signature burned, or unknown+expired-blockhash); leave unknown+valid alone (dedup-safe to resend same tx). Slip: "failed -> resubmit same tx" (wrong - signature consumed, must rebuild). Good clarifying Qs (SOL-not-SPL challenge was correct, caught the interviewer). **Meta-lesson: default instinct = live loop; needs to become = write-ahead log when problem says "never double X across a crash".** |
