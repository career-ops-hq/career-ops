...
export function trackedFiles(root, filter = () => true) {
  ...
  //               throws EISDIR.
  //
  //   UNSTATTABLE `throwIfNoEntry: false` suppresses ENOENT only. The index
  //               naming `a/b` while the tree has `a` as a regular file throws
  //               ENOTDIR, and an unreadable parent directory throws EACCES.
  //               Both are the MISSING case wearing a different errno, so both
  //               are skipped rather than allowed to end the scan.
  //
