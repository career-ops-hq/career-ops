...
export function trackedFiles(root, filter = () => true) {
  ...
  // A repository-local `core.hooksPath` is not consulted here: this shells out to
  // `git ls-files`, which reads the index and runs no hook.
  ...
  //               throws EISDIR.
  //
  //   UNSTATTABLE `throwIfNoEntry: false` suppresses ENOENT only. On POSIX the
  //               index naming `a/b` while the tree has `a` as a regular file
  //               throws ENOTDIR, and an unreadable parent directory throws
  //               EACCES. Both are the MISSING case wearing a different errno,
  //               so both are skipped rather than allowed to end the scan.
  //               Windows reports the first of those as ERROR_PATH_NOT_FOUND,
  //               which libuv maps to ENOENT, so there it arrives through the
  //               MISSING branch below instead (#3904). Same outcome either
  //               way, which is why the branches share one warning.
  //
  ...
  for (const rel of listed) {
    ...
    let stat;
    try {
      stat = lstatSync(join(root, rel), { throwIfNoEntry: false });
    } catch (err) {
      // `throwIfNoEntry: false` suppresses ENOENT and nothing else, so the
      // UNSTATTABLE case above still arrives as a throw. Uncaught it takes the
      // whole scan with it, which is the failure this function exists to
      // prevent. Named with the errno, since the reason is the errno.
