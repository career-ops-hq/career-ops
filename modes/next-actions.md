...
5. Use `add` for one manual request, or preview a structured `import --file ... --dry-run` and import proposals. Accept only a reviewed task. An explicit user instruction to add that action already authorizes recording its acceptance; do not ask again. Inferred obligations remain proposals until reviewed.
