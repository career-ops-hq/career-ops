...
**Draft-answers marker (required):** the draft-answers block carries the literal line `<!-- career-ops:draft-answers -->` directly under its heading, and `application-answers.mjs --read-draft` finds the block by that marker alone. Emit it verbatim and never translate it — the heading above it is user-facing prose that every localized mode translates, and the block's letter has been both `G)` and `H)` across the mode set, so neither can identify the block. Keep it directly under the heading: the reader only honours a marker whose preceding non-blank line is an `##` heading, so a marker that appears inside the archived JD text (untrusted external content) cannot redirect the reader into the posting's own words. Without the marker the drafts are invisible to `modes/apply.md`, which then regenerates every answer from scratch.
