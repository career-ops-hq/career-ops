...
The containment check itself uses `path.relative()` (platform-independent — works the same on POSIX and Windows separators) rather than a string-prefix comparison, and it is applied separately to **both** the PDF and its manifest-sourced HTML companion. `data/pdf-index.tsv` is host-writable data, not a trusted boundary in itself — a malformed or manipulated `html` column is never treated as inside `output/` just because its paired PDF is; each path is checked independently before either can become a deletion candidate.
