...
15. Crea `output/{company}-{YYYY-MM-DD}/` si no existe. Ejecuta: `node generate-pdf.mjs /tmp/cv-{candidate}-{company}.html output/{company}-{YYYY-MM-DD}/cv-{candidate}.pdf --format={letter|a4}`
...
- **Fonts**: Sora (headings, 600-700) + Inter (body, 400-500)
- **Fonts**: loaded via Google Fonts @import in template
...
| `{{AVATAR_PATH}}` | Base64 data URI of avatar image (embed with `data:image/jpeg;base64,...`) |
| `{{HEADER_SUBTITLE}}` | Role-specific subtitle (e.g. "Solana Smart Contracts Engineer") |
| `{{SECTION_COMPETENCIES}}` | Core Skills / Competencias Core |
...
b. **IMMEDIATELY** download the PDF using Bash:
   ```bash
   mkdir -p "output/{company}-{YYYY-MM-DD}"
   curl -sL -o "output/{company}-{YYYY-MM-DD}/cv-{candidate}-canva.pdf" "{download_url}"
   ```
   The export URL is a pre-signed S3 link that expires in ~2 hours. Download it right away.
c. Verify the download:
   ```bash
   file output/{company}-{YYYY-MM-DD}/cv-{candidate}-canva.pdf
   ```
