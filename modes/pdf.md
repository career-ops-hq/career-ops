...
17b. Run the structure check against the payload: `node verify-cv-structure.mjs /tmp/cv-{candidate}-{company}.json`
    - This is a warning, not a hard gate, before HTML is even built. `verify-cv-facts.mjs` (step 19) blocks *fabrication* — a claim absent from cv.md; this check flags the opposite failure, which fabrication-checking cannot see: cv.md content silently *lost* or *reordered* while tailoring. It catches a paid-employer entry omitted entirely, an experience entry's company descriptor (`"Austin, TX · Series B fintech"`) getting dropped down to a bare city/state, and entries rendered out of cv.md's own chronological order. A heading that explicitly names a Career Break, sabbatical, or leave remains exempt from the full-entry omission warning.
