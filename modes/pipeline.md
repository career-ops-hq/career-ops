...
**A non-zero exit means PROCEED.** Check the exit code first; on exit 0, branch on the printed verdict because both real verdicts exit 0. Exit 1 is a malformed invocation (an unknown flag, a missing operand): fix the command and rerun it once, and proceed if it still fails. Exit 2 is a JD file that could not be read, so there is no pre-score: send the URL on to the pre-screen gate. Exit 3 is a proposed `skip` that could not be written to `data/discard.log`: stdout also reports `proceed` with `override: log-failed` in JSON. Do **not** mark that posting skipped. A posting is never dropped without its audit line; a skip nobody can audit is the silent black box the discard log exists to prevent.
...
- **A skip always rests on evidence.** If every signal came back unknown the verdict is `proceed` whatever the threshold (`override: no-evidence`). The same applies when the gate is not configured: no readable `config/profile.yml` or `cv.md`, or no usable `target_roles.primary` / `target_roles.archetypes[].name` in YAML, means `override: not-configured` and a proceed. Targeting kept only in `modes/_profile.md` still reaches full evaluation; this gate does not parse that prose.
