...
2. **For each surviving pending URL**:
   ...
   e. **Execute full auto-pipeline**: Evaluation A-F → Report .md → PDF (if score >= `auto_pdf_score_threshold`) → Tracker. Read `modes/_custom.md` in full before processing and apply every relevant override, including the CV/PDF template and formatting contract. The PDF step must use the configured `cv.template`; do not substitute a base or alternate template unless explicitly requested. Default (if absent or silent): standard pipeline execution.
