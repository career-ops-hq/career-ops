...
// Usage: node plugins/h1b-sponsor/check.mjs <company-name> [--json|--summary] [--refresh]
...
function parseArgs(argv) {
  const args = { name: null, format: 'summary', refresh: false };
  const rest = [];
  for (const a of argv) {
...
async function main() {
  const { name, format, refresh } = parseArgs(process.argv.slice(2));
  if (!name) {
    process.stderr.write('Usage: node plugins/h1b-sponsor/check.mjs <company-name> [--json|--summary] [--refresh]\n');
    process.exit(2);
  }
  ...
  if (!match) {
    await writeCache(name, { name }, { negative: true });
    const out = notFoundResult(name);
    emit(format, out);
    return;
  }
