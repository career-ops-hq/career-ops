...
// ── 45d. companyMatch strips CJK/Korean corporate-form markers (#2570) ──
// Follow-up to 45c: 株式会社メルカリ vs メルカリ survived the #2445/#2569 fix
// unmatched, because the character before メルカリ in 株式会社メルカリ is 社,
// a letter — no anchor rule can treat that as a boundary, and CJK/Korean
// don't delimit words with spaces the way LEGAL_SUFFIXES (invite-match.mjs)
// assumes. The fix strips a known corporate-form marker as an unspaced
// prefix or suffix before the equality/containment checks run.
console.log('\n45d. companyMatch strips CJK/Korean corporate-form markers (#2570)');
try {
  ...
  // Regression: Latin behavior from 45c must be untouched by a CJK-only list.
  if (companyMatch('Acme Inc.', 'acme inc') === true && companyMatch('Acme', 'Zoro Inc') === false) {
