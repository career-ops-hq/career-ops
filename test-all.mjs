...
// ── 44g. evaluators — the score cell is parsed, never concatenated ────
// gemini-eval's normalizedTrackerScore appended `/5` to whatever the summary
// carried. validateReport only checks the SCORE line's numeric PREFIX
// (gemini-eval.mjs:257) while extract() returns the whole rest of the line
// (gemini-eval.mjs:446), so `SCORE: 4.2 (strong fit)` passed validation and
// became the cell `4.2 (strong fit)/5` — not a score by SCORE_CELL_RE and not a
// sentinel, i.e. exactly the ambiguous row merge-tracker refuses. Its sibling
// batch-evaluate-gemini.mjs parsed the float and was never affected; nothing
// held the copies to one behavior.
//
// The evaluators run on import (arg parse + fetch), so lift each copy out of the
// source and exercise it directly. Discovery is by definition, not by a fixed
// list: a new evaluator that grows its own copy is tested the day it lands.
console.log('\n44g. evaluators — normalizedTrackerScore emits a parseable score cell');
try {
  const { looksLikeScoreCell } = await import(pathToFileURL(join(ROOT, 'tracker-parse.mjs')).href);
  const helperRe = (name) => new RegExp(`^function ${name}\\([\\s\\S]*?\\n\\}`, 'm');
  const evaluators = ['gemini-eval.mjs', 'batch-evaluate-gemini.mjs', 'openai-eval.mjs', 'ollama-eval.mjs']
    .filter(name => existsSync(join(ROOT, name)))
    .map(name => [name, readFile(name)])
    .filter(([, src]) => helperRe('normalizedTrackerScore').test(src));

  // A score line the model wrote with trailing prose, plus the sentinels every
  // copy has to agree on. Expectations are the behavior, not one file's code.
  const cases = [
    ['4.2',                '4.2/5'],
    ['4.2/5',              '4.2/5'],
    ['4.2 (strong fit)',   '4.2/5'],
    ['4.5 — strong match', '4.5/5'],
    ['5',                  '5/5'],
    ['?',                  'N/A'],
    ['',                   'N/A'],
    [undefined,            'N/A'],
  ...
  const problems = [];
  for (const [name, src] of evaluators) {
    ...
    for (const [input, expected] of cases) {
      const got = fn(input);
      if (got !== expected) problems.push(`${name}: ${JSON.stringify(input)} -> ${JSON.stringify(got)}, expected ${JSON.stringify(expected)}`);
      // The cell has to survive the reader, whatever the copy decided to return.
      if (!looksLikeScoreCell(got)) problems.push(`${name}: ${JSON.stringify(input)} -> ${JSON.stringify(got)}, which looksLikeScoreCell rejects`);
    }
  }

  if (evaluators.length === 0) {
    fail('no evaluator defines normalizedTrackerScore — the helper moved, so this test is no longer guarding anything');
  } else if (problems.length === 0) {
    pass(`all ${evaluators.length} normalizedTrackerScore copies parse the score and emit a cell every reader accepts`);
  } else {
    fail(`score-cell normalization broken: ${problems.join('; ')}`);
  }
} catch (e) {
  fail(`evaluator score-cell test crashed: ${e.message}`);
}
