...
//
// `.github/` stays outside the enforced set, for a structural reason rather
// than a preference. Every workflow that runs a script from `.github/scripts/`
// checks out with `sparse-checkout: .github/scripts`, so `lib/` is not on disk
// when the script runs. A static `import { isMainModule } from
// '../../lib/is-main-module.mjs'` throws ERR_MODULE_NOT_FOUND there and takes
// the `direction-gate` required check down with it. That is the
// update-system.mjs self-loading constraint above, arriving from a workflow
// instead of from #1706. One of those files cannot be edited here at all:
// `.github/scripts/triage.bundle.mjs` is generated by
...
// exempt path off disk. The `files.length > 100` floor below still fails if
// this predicate ever widens into swallowing the tree.
const repoSources = () =>
  trackedFiles(ROOT, (rel) => rel.endsWith('.mjs') && !rel.startsWith('.github/'));
