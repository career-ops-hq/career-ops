...
async function apply() {
  ...
  try {
    ...
    for (const path of updatePaths) {
      ...
      if (pathFullyPreserved(path, preservedPaths, preservedSet, { ref: pairedTargetRef })) continue;
      try {
        ...
        try { gitQuiet('cat-file', '-e', `${pairedTargetRef}:${spec}`); }
    ...
    const unmaterialized = missingFromTargetManifest(remoteSystemPaths, pairedTargetRef);
