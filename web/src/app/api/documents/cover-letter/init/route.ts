...
export async function POST(request: Request) {
  let body: { applicationId?: unknown; explicit?: unknown };
  ...
  try {
    const enabled = typeof body.explicit === "boolean" ? body.explicit : autoCoverEnabled(custom);
    return NextResponse.json(initializeCoverLetterDraft(careerOpsRoot(), applicationId, enabled));
  } catch {
