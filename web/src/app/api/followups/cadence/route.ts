...
import { careerOpsRoot, rootScript, coreCheckoutRoot } from "@/lib/career-ops";
...
async function readCoreDefaults(): Promise<Partial<Record<ProfileCadenceKey, number>> | null> {
  ...
  const stdout = await new Promise<string>((resolve) => {
    execFile("node", [script, "--json"], { cwd: coreCheckoutRoot(), timeout: 12_000 }, (_e, out) => resolve(out || ""));
  });
  try {
...
export async function POST(req: Request) {
  ...
  if (!fs.existsSync(file)) {
    ...
    try {
      const seeded = yaml.load(fs.readFileSync(path.join(coreCheckoutRoot(), "config", "profile.example.yml"), "utf8"));
