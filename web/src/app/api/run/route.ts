...
export async function POST(req: Request) {
  ...
  // The evaluate precondition must check the market-specific file the prompt
  // actually reads. CAREER_OPS_ROOT is runtime user data, not a build input; keep
  // dynamic path access ignored by Turbopack so server bundles do not ingest the
  // whole local checkout.
  const lang = readLanguageConfig();
  const needsScripts: Record<string, string[]> = {
    evaluate: [lang.evalModeFile],
    "fix-portal": ["verify-portals.mjs"],
    pdf: ["build-cv-html.mjs", "generate-pdf.mjs"],
  };
  const missing = (needsScripts[kind] ?? []).find((required) => {
    const requiredPath = path.join(/* turbopackIgnore: true */ careerOpsRoot(), required);
    return !fs.existsSync(/* turbopackIgnore: true */ requiredPath);
  });
  if (missing) {
    return new Response(
      JSON.stringify({
