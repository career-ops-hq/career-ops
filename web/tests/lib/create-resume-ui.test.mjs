...
test("Create New Version reaches the shared backend role source loader", () => { assert.match(route, /loadRoleResumeSource\(careerOpsRoot\(\)\)/); assert.match(route, /roleSourceCv: roleSource\?\.cv/); });
test("Create New Version sends General Role prompts through stdin", () => { assert.match(route, /promptViaStdin = cliId === "codex" && \(kind === "role-resume" \|\| !!manualJob\)/); assert.match(route, /stdinMode: "pipe", stdinInput: prompt/); });
